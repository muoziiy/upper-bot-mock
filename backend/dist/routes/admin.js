...
router.post('/approve-request', async (req, res) => {
    ...
    try {
        ...
        const { error: updateReqError } = await supabase_1.supabase
            .from('registration_requests')
        ...
        if (logs && logs.length > 0) {
            for (const log of logs) {
                try {
                    await bot_1.default.telegram.editMessageText(log.admin_chat_id, Number(log.message_id), undefined, `✅ Request approved.`, { parse_mode: 'Markdown' } // Removes buttons by not including reply_markup
                    );
                }
                catch (e) {
...
router.post('/decline-request', async (req, res) => {
    ...
    try {
        ...
        if (logs && logs.length > 0) {
            for (const log of logs) {
                try {
                    await bot_1.default.telegram.editMessageText(log.admin_chat_id, Number(log.message_id), undefined, `❌ Request declined.`, { parse_mode: 'Markdown' });
                }
                catch (e) {
                    console.error(`Failed to update message for admin ${log.admin_chat_id}`, e);
                }
...
router.get('/stats/financial', async (req, res) => {
    try {
        ...
        let totalOutgoing = 0;
        try {
            const { data: outgoingData, error: outgoingError } = await supabase_1.supabase
        ...
        const { data: recentIncoming, error: incomingError } = await supabase_1.supabase
            .from('payment_records')
            .select(`
                id,
                amount,
        ...
        let recentOutgoing = [];
        try {
            const { data: outgoing, error: outError } = await supabase_1.supabase
                .from('teacher_payments')
                .select(`
                    id,
                    amount,
...
router.get('/students', async (req, res) => {
    ...
    try {
        ...
        const students = studentsData.map((student) => {
            ...
            let hasPaid = false;
            const groups = student.group_members?.map((gm) => {
...
router.get('/students/:id', async (req, res) => {
    ...
    try {
        ...
        if (studentError)
            throw studentError;
        // 2. Fetch groups
        const { data: groupsData, error: groupsError } = await supabase_1.supabase
            .from('group_members')
            .select(`
                joined_at,
                payment_status,
...
router.post('/attendance', async (req, res) => {
    ...
    try {
        // 1. Record Attendance
        const { error: attendanceError } = await supabase_1.supabase
            .from('attendance')
            .insert({
            student_id,
            group_id,
        ...
        if (status === 'present') {
            const { data: groupMember, error: gmError } = await supabase_1.supabase
                .from('group_members')
                .select(`
                    lessons_remaining,
                    groups (payment_type)
            ...
            if (group?.payment_type === 'lesson_based') {
                ...
                if (newCredits <= 2) {
                    ...
                    if (student?.telegram_id) {
                        ...
                        try {
                            await (0, notifications_1.sendBroadcastNotification)([student.telegram_id], `⚠️ *Low Balance Warning*\n\nYou have ${newCredits} credits left for this group. Please top up soon.`);
                        }
                        catch (notifError) {
...
router.post('/broadcast', async (req, res) => {
    ...
    try {
        ...
        if (target_type === 'all_students') {
            const { data } = await supabase_1.supabase.from('users').select('telegram_id').eq('role', 'student');
            recipients = data || [];
        }
        else if (target_type === 'all_teachers') {
            const { data } = await supabase_1.supabase.from('users').select('telegram_id').eq('role', 'teacher');
            recipients = data || [];
        }
        else if (target_type === 'all_admins') {
            const { data } = await supabase_1.supabase.from('users').select('telegram_id').eq('role', 'admin');
            recipients = data || [];
        }
...
router.post('/settings/payment-type', async (req, res) => {
    ...
    try {
        ...
        await supabase_1.supabase.from('education_center_settings').update({ default_payment_type: payment_type }).gt('updated_at', '2000-01-01');
...
// HELPER: RECALCULATE STUDENT GROUP STATUS
// ==============================================================================
async function recalculateStudentGroupStatus(studentId, groupId) {
    try {
        ...
        if (paymentType === 'lesson_based') {
            ...
            const { count: totalUsedLessons } = await supabase_1.supabase
                .from('attendance_records')
                .select('*', { count: 'exact', head: true })
                .eq('student_id', studentId)
                .eq('group_id', groupId)
